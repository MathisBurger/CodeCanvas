...
import {Spotlight, SpotlightActionGroupData, SpotlightFilterFunction} from "@mantine/spotlight";
...
const SpotlightWrapper = () => {
  ...
  const {t} = useTranslation('common')
    ...
    const filter: SpotlightFilterFunction = (query: string, actions: (SpotlightActionGroupData|SpotlightActionData)[]): (SpotlightActionGroupData|SpotlightActionData)[] => {
      return actions.filter((group: any) => {
          group.actions = group.actions.filter((action: any) => {
              return action.id.indexOf("stage3") > -1
