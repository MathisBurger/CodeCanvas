...
const CreateQuestionsModal = ({onClose, groupId, assignmentId, refetch}: CreateQuestionsModalProps) => {

    const api = useApiServiceClient();
    const [questions, setQuestions] = useState<QuestionCatalogueElement[]>([]);

    const updateQuestion = useCallback((index: number, value: QuestionCatalogueElement) => {
    ...
    const addNew = () => {
        setQuestions([...questions, {answer: '', answer_type: null, question: ''}]);
    }
    ...
    const onSubmit = async () => {
        for (const question of questions) {
            if (isEmpty(question.question) || isEmpty(question.answer) || isEmpty(question.answer_type)) {
    ...
    return (
      <Modal opened={true} onClose={onClose} title="Create new questions" size="xl">
          <Stack>
              ...
              <Group justify="flex-end">
                  <Button onClick={addNew} w={150}>New question</Button>
              </Group>
          </Stack>
          <Divider mt={10} />
          <Group mt={10}>
              <Button type="submit" onClick={onSubmit}>Create questions</Button>
