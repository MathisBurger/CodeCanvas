...
const CreateOrUpdateAssignmentModal = ({groupId, onClose, refetch, assignment, action}: CreateAssignmentModalProps) => {
    ...
    const onSubmit = form.onSubmit(async (values) => {
        try {
            if (action === "create") {
                const res = await api.createAssignment(groupId, values.title, values.due_date, values.description, values.language);
                notifications.show({
            ...
            if (action === "update") {
                const res = await api.updateAssignment(groupId, assignment?.id ?? -1, values.title, values.due_date, values.description);
                notifications.show({
    ...
    return (
        <Modal opened onClose={onClose} title={action === 'create' ? 'Create Assignment' : 'Update Assignment'} size="xl">
            <form onSubmit={onSubmit}>
                ...
                />
                <RichTextInput
                    content={assignment?.description ?? form.getInputProps('description').value}
