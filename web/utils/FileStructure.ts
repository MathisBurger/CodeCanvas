...
import {DisplayMode, FileStructureFile, FileStructureTree} from "@/components/FileStructure";
 ...
 */
export const buildDataFromStructure = (structure: FileStructureTree, fileRoot: string, editable: boolean): [string[], TreeNodeData[]] => {
    const data = [];
    const fileNames = [];
 ...
 */
export const createFolder = (structure: FileStructureTree, path: string, folderName: string): FileStructureTree => {
 ...
 */
export const createFile = (structure: FileStructureTree, path: string, fileName: string): FileStructureTree => {
    const spl = path.split('/');
    if (spl.length === 1) {
 ...
 */
export const updateTestFileState = (structure: FileStructureTree, path: string, fileName: string, state: boolean): FileStructureTree => {
    const spl = path.split('/');
    if (spl.length === 2) {
        // @ts-ignore
 ...
 */
export const findObjectIdInStructure = (structure: FileStructureTree, path: string): FileStructureFile|null => {
    const spl = path.split('/');
    if (spl.length === 2) {
        for (const file of structure.files) {
 ...
 */
export const filterFileStructureForDisplayMode = (structure: FileStructureTree, displayMode: DisplayMode): FileStructureTree => {
    if (displayMode === 'all') {
        return structure;
    }
    structure.files = structure.files.filter((f) => displayMode === 'test' ? f.is_test_file : !f.is_test_file);
    const newFolders = [];
    for (const folder of (structure.folders ?? [])) {
